...
pub use tezos_smart_rollup_encoding::dac::{
    prepare_preimages, Page, PreimageHash, V0ContentPage, V0HashPage,
};
...
pub fn make_preimage_hash(
    content: &[u8],
) -> Result<[u8; PREIMAGE_HASH_SIZE], tezos_crypto_rs::blake2b::Blake2bError> {
    if content.len() > MAX_PAGE_SIZE {
        return Err(tezos_crypto_rs::blake2b::Blake2bError::Other);
    }

    let hash = tezos_crypto_rs::blake2b::digest_256(content)?;
...
pub mod encoding {
    ...
    ) -> Result<PreimageHash, tezos_crypto_rs::blake2b::Blake2bError> {
...
mod tests {
    ...
    const EXAMPLE_CONTENT_PAGE: &[u8] = &[
        0, 0, 0, 0, b'A', b'L', b'o', b'r', b'e', b'm', b' ', b'i', b'p', b's', b'u', b'm', b' ',
        b'd', b'o', b'l', b'o', b'r', b' ', b's', b'i', b't', b' ', b'a', b'm', b'e', b't', b',',
        b' ', b'c', b'o', b'n', b's', b'e', b'c', b't', b'e', b't', b'u', b'r', b' ', b'a', b'd',
        b'i', b'p', b'i', b's', b'c', b'i', b'n', b'g', b' ', b'e', b'l', b'i', b't', b',', b' ',
        b's', b'e', b'd', b' ', b'd', b'o', b' ', b'e',
    ];

    // taken from DAC test example in tezos
    const EXAMPLE_HASH_PAGE: &[u8] = &[
        1, 0, 0, 0, b'B', 0, b'r', 180, b'a', b'2', b'Z', b'(', 220, 14, 4, 220, b'{', b'N', b'n',
        b'@', 183, b'#', b'!', 6, b'm', 204, b'p', 130, 162, 247, 246, 16, b'l', 239, b'7', b'"',
        249, 163, 0, 155, 167, 146, 19, 175, 28, b'V', 129, 247, 208, 31, b'F', b'd', 183, 194,
        149, b'H', 163, b'|', 246, 164, 201, b'&', 195, 129, 24, 3, b'}', b'4', b't', 11, 213,
    ];

    #[test]
    fn encode_decode_hash_page() {
        let (_, page) = Page::nom_read(EXAMPLE_HASH_PAGE).expect("Deserialization should work");
    ...
    fn encode_decode_contents_page() {
        let (_, page) = Page::nom_read(EXAMPLE_CONTENT_PAGE).expect("Deserialization should work");
        let mut buffer = Vec::new();
        page.bin_write(&mut buffer)
            .expect("Serialization should work");
    ...
    fn decoding_contents_over_slice() {
        ...
        let slice_page = SlicePage::try_from(EXAMPLE_CONTENT_PAGE).expect("Should be content page");
    ...
    fn decoding_hash_over_slice() {
        ...
        let slice_page = SlicePage::try_from(EXAMPLE_HASH_PAGE).expect("Should be hash page");
        ...
        match (&page, &slice_page) {
            (Page::V0HashPage(page), SlicePage::V0HashPage(slice_page)) => {
                ...
                let slice_hashes: Vec<&[u8; PREIMAGE_HASH_SIZE]> = slice_page.hashes().collect();
    ...
    fn save_content<Host: tezos_smart_rollup::host::Runtime>(
        buffer: &mut Vec<u8>,
