...
async function main() {
  ...
  let twitter = window.localStorage.getItem(twitter_path); 
  if (!secret || !twitter) {
    alert("You must register before playing. Navigating to entrypoint now...");
  ...
  window.document.getElementById("tz1").innerText = await tezos.wallet.pkh();
  ...
  let place = new Place(glWindow, tezos, signer);
...
const GUI = (cvs, glWindow, place) => {
  ...
  colorField.addEventListener("change", (ev) => {
    let hex = (colorField as any).value
      .replace(/[^A-Fa-f0-9]/g, "")
      .toUpperCase();
